...
require "electrical_voxelcraft:block_extensions";

function machine_give_energy(x, y, z, vce)
    local data = get_block_data(x,y,z)
    if vce > data.max_voltage then 
        explode(x, y, z, data.explode_strength) return 0 
    end
...
function machine_update(x, y, z)
    local data = get_block_data(x,y,z)
    if block.get_name(x, y, z):startsWith(
        "electrical_voxelcraft:machine_light"
    ) then
        if data.energy_bank_value >= data.work_voltage then
            data.energy_bank_value = data.energy_bank_value - data.work_voltage
            if block.get_name(
                x, y, z
            ) == "electrical_voxelcraft:machine_light" then
                block.set_with_name(
                    x, y, z, "electrical_voxelcraft:machine_light_active"
                )
            end
        else
            if block.get_name(
                x, y, z
            ) == "electrical_voxelcraft:machine_light_active" then
                block.set_with_name(
                    x, y, z, "electrical_voxelcraft:machine_light"
                )
            end
        end
    end
